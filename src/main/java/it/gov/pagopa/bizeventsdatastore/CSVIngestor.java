...
public class CSVIngestor {
    ...
    private final Logger logger = LoggerFactory.getLogger(CSVIngestor.class);

    private final MassiveBizViewRegenQueueService massiveBizViewRegenQueueService;

    CSVIngestor(MassiveBizViewRegenQueueService massiveBizViewRegenQueueService) {
        this.massiveBizViewRegenQueueService = massiveBizViewRegenQueueService;
    }

    public CSVIngestor() {
        this.massiveBizViewRegenQueueService = new MassiveBizViewRegenQueueServiceImpl();
    }

    /**
     * This function will be invoked when a Http Trigger occurs
     */
    @FunctionName("CSVIngestor")
    public HttpResponseMessage run(
        ...
        try (CSVReader reader = new CSVReaderBuilder(new InputStreamReader(body.get())).build()) {
            ...
            while ((line = reader.readNext()) != null) {
                ...
                boolean result = this.massiveBizViewRegenQueueService.sendBizEventIdToQueue(bizEventId);
                if (!result) {
