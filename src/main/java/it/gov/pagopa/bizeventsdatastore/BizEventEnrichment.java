...
public class BizEventEnrichment {
	...
	public void processBizEventEnrichment(
			@EventHubTrigger(
					...
					consumerGroup = "BizViewEnrichment")
			List<BizEvent> items,
		...
		if (retryIndex >= EBR_MAX_RETRY_COUNT) {
			bizEventDeadLetterService.handleLastRetry(context, id, executionDateTime, "last-retry-input", items,
		...
		try {
			if (items.size() == properties.length) {
				for (int i=0; i<items.size(); i++) {
					...
					if (Strings.isNullOrEmpty(cachedValue) /*enableTransactionListView lo lasciamo il flag?*/) {
			...
			if (retryIndex >= EBR_MAX_RETRY_COUNT) {
				bizEventDeadLetterService.handleLastRetry(context, id, executionDateTime, "exception-output", bizEvtMsgWithProperties, CONTAINER_BIZ_EVENTS_VIEWS_DEAD_LETTER_NAME, telemetryClient);
			}
