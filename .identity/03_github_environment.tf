...
#tfsec:ignore:github-actions-no-plain-text-action-secrets 
resource "github_actions_environment_secret" "secret_integration_test_cosmos_biz_key" {
  count  = var.env_short != "p" ? 1 : 0
  repository       = local.github.repository
  environment      = var.env
  secret_name      = "COSMOS_DB_PRIMARY_KEY"
  plaintext_value  = data.azurerm_key_vault_secret.key_vault_integration_cosmos_biz_key[0].value
}

#tfsec:ignore:github-actions-no-plain-text-action-secrets 
resource "github_actions_environment_secret" "secret_integration_test_ehub_tx_biz_key" {
  count  = var.env_short != "p" ? 1 : 0
  repository       = local.github.repository
  environment      = var.env
